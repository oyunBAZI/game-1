...
The scene includes articulated athletes with procedural sportswear detail, laminated rackets, a regulation table, a woven deformable net, textured competition flooring, arena seating and a live 3D scoreboard. Lighting uses room reflections, venue fixtures and soft shadows; the ball has a height-sensitive table shadow. The 240 Hz simulation resolves swept contacts in time order against the full tabletop, including moving tilted rackets, top edges, vertical apron and the net cord. Apron hits lose the point rather than becoming legal bounces. Input is sampled immediately before physics in a single frame loop. Legal net serves replay as lets, and the AI predicts with the same physics as the live ball. `npm run typecheck`, `npm run test:simulation`, `npm run validate`, and `npm run build` are the project gates. Coefficients are gameplay approximations; measured equipment calibration and authored human models remain future work.

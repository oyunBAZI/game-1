...
The scene includes articulated athletes and rackets, a textured regulation table with braced undercarriage, a woven deformable net, a lit indoor court with live arena scoreboard, branded barriers, and instanced seats and spectators. The simulation uses a 240 Hz fixed step, time-ordered swept paddle/table/net contacts, bounded aerodynamic lift, contact impulse and spin, serve bounce rules, scoring, and an AI return with a shared flight predictor. `npm run typecheck`, `npm run test:simulation`, `npm run validate`, and `npm run build` are the project gates. Coefficients are gameplay approximations; measured real-world calibration and authored character assets remain future work.

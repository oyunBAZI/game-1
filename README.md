...
This repository is an evolving Three.js table tennis game. Run `npm ci` and `npm run dev`, open the local URL, choose a serve spin and venue, select **Start session**, and press **Enter** to serve. Move with WASD, aim the paddle with the pointer, hold Space or the pointer button to swing, press C for camera modes, and Escape to pause or change the serve selection. The opponent serves automatically on its turn. Audio unlocks on the first pointer press. World Finals is the initial venue; the other four courts remain selectable.

### World Finals and impact pass

- The new World Finals has an overhead copper lighting ring, woven hanging pennants, gallery architecture, a dense championship floor finish, court crests, warm practical light and packed spectator stands. Its architecture, markings and lighting are distinct from the other four venues. Repeated stands and ceiling detail continue to use instancing.
- Moving athletes now cycle their planted and recovering feet according to distance travelled, including lateral steps, lifted soles and corresponding contact shadows. Standing still no longer runs a clock-driven walking pose.
- Continuous contacts now restore the pre-probe ball state and integrate to the time of impact before resolving the rebound. That preserves transient forces and produces impact speed from the actual impact state; the remaining fraction of the frame is integrated after the contact. This runs in both live play and the AI's physics predictor.
- An out-of-play ball remains animated during the point delay, can strike the net or table, and bounces and settles on the floor. It cannot award another point. Scoring happens when its center reaches floor level, rather than after the ball disappears beneath the court.
- The presentation settings now offer low, medium, high and ultra graphics presets. The selected pixel density and shadow resolution take effect immediately without changing the 240 Hz simulation or resetting a rally.
- Regression checks compare coarse and fine impact steps, verify post-point settling without duplicate scoring, and construct the championship scene. Run `npm run typecheck && npm run test:simulation && npm run validate && npm run build` before committing.

The player models and venue artwork remain hand-built procedural meshes and textures. They are richer game assets, but photorealistic scanned characters, authored motion capture, calibrated hardware and a console engine's rendering pipeline would require a dedicated art and engineering production effort. A GPU visual inspection is still recommended before calling the result production quality.

The scene includes articulated athletes with sculpted torsos and procedural sportswear, laminated rackets, a beveled regulation table, a woven deformable net, textured competition flooring, arena seating and a live 3D scoreboard. Choose between the Training Lab, Club Hall, National Arena and Night Court; each changes the venue palette, crowd, lighting, signage and set dressing. An instanced coffer ceiling and venue specific architecture add depth beyond the court. Lighting uses room reflections, venue fixtures and soft shadows; the ball and athletes have soft contact shadows and real table impacts produce brief surface cues. The 240 Hz simulation resolves swept contacts in time order against the full tabletop, including rotating and translating rackets, top edges, vertical apron, the net cord and the two posts. Midpoint integration evaluates drag and Magnus lift partway through each flight step. Brushing spin comes from a tangential rubber friction impulse rather than directly modifying the ball's angular velocity. Apron hits lose the point rather than becoming legal bounces. Input is sampled immediately before physics in a single frame loop. Legal net serves replay as lets, and the AI predicts with the same physics as the live ball, remembering the bounce that happened before its forecast began. `npm run typecheck`, `npm run test:simulation`, `npm run validate`, and `npm run build` are the project gates. Coefficients are gameplay approximations; scanned human models, authored animation and measured equipment calibration remain future work.
...
### Latest play and presentation pass

- Every serve begins with a visible 0.42-second toss beside the server. Its initial velocity is fitted by a bounded shooting method against the same aerodynamic, net and tabletop code used in a match. The selected first bounce, receiver bounce and lateral target now match the real simulated landings closely across flat, topspin, backspin, pendulum, kick and soft serves. Both players' rackets stay inactive until a legal receiver bounce.
- Racket grips are limited to an arm-length reach from the athlete's leading shoulder. The articulated arm and collider use the same shoulder and grip positions, eliminating impossible long-distance returns and stretched-looking arm poses. Club AI still returns reachable serves; its reaction time and footwork remain the main difficulty controls.
- Spectators in the National Arena now have instanced arms and varied hair silhouettes, and the audience and venue lights react briefly to points. Reduced motion prevents the crowd and light pulse. Racket, table and net impacts use short filtered noise and resonant tones with lateral stereo placement; sounds are generated locally and only start after a user gesture.
- Simulation checks cover the toss, landing accuracy and net clearance for every serve type from both sides, racket reach and the audience's return to its resting pose. The production build and static validation remain required checks.
